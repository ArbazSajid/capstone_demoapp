namespace :ptourist do
  ...
  def create_tag organizer, tag
    puts "building tag for #{tag[:value]}, by #{organizer.name}"
    tag=Tag.create(:creator_id=>organizer.id,:value=>tag[:value])
  ...
  def create_thing thing, organizer, members, images, tags
    ...
    tags.each do |tag|
      puts "building tag for #{thing.name}, #{tag[:value]}, by #{organizer.name}"
      tag=Tag.create(:creator_id=>organizer.id,:value=>tag[:value])
      organizer.add_role(Role::ORGANIZER, tag).save
      TagThing.new(:thing=>thing, :tag=>tag, 
  ...
  task delete_subjects: :environment do
    puts "removing #{Thing.count} things and #{ThingImage.count} thing_images and #{TagThing.count} tag_things "
    puts "removing #{Image.count} images"
    puts "removing #{Tag.count} tags"
...
"}
   ...
   organizer=get_user("marsha")
    ...
    puts "#{Thing.count} things created and #{ThingImage.count("distinct thing_id")} with images  and #{TagThing.count("distinct thing_id")} with tags"
    puts "#{Image.count} images created and #{ThingImage.count("distinct image_id")} for things"
    puts "#{Tag.count} tags created and #{TagThing.count("distinct tag_id")} for things"
  end
